...
## Configure buffering {.tabset}

### EdgeCmd Utility
Buffer configuration using EdgeCmd Utility will follow the same format as updating other facets within a component. 

1. Using a text editor, create an empty text file.
2. Copy and past an example configuration for buffering into the file.
    ...
    ```
3. Update the example JSON parameter for your environment.
...
6. Enter the following command to initialize the buffering configuration.

    ```bash
    edgecmd -port 5590 set Buffering -file C:\Users\TestUser\OpcUa1\ConfigureBuffering.json
    ```
    **Note:** If you installed the adapter to listen to a port other than the default 5590 port, update the command to reflect the appropriate port number.
...
```bash
`%ProgramFiles%\OSIsoft\Adapters\<AdapterName>\Schemas`
```
**Linux**

```bash
`/opt/OSIsoft/Adapters/<AdapterName>/Schemas`
```
</br>
<hr style="border:2px solid gray">
